...
- **Auto-fill from the screenshot:** drop a BookMyShow/PVR-style ticket and it's read
  **on-device** (Tesseract.js OCR, in the browser — the image never leaves your phone) to
  ...
  offline.
- **Add many:** from the `⋯` menu (or by dropping several files on the Add sheet), pick
  **multiple images at once** — each is read and **auto-classified** onto the right shelf
  (Movies / Concerts / Events / One-off) and dated, then a quick review list lets you fix
  any title, shelf or date before adding them all in one go.
